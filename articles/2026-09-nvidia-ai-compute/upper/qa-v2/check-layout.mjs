...
for(const width of [360,390,430,1100]){
 ...
 await page.goto(pathToFileURL(path.join(upper,'article-v2.html')).href);
 ...
 if(width===390){
  await page.screenshot({path:path.join(upper,'qa-v2/mobile-full.png'),fullPage:true});
  await page.screenshot({path:path.join(upper,'qa-v2/mobile-opening.png')});
  for(const [i,name] of [[0,'gpu-specs'],[1,'launch-prices']]){
   ...
   await page.screenshot({path:path.join(upper,`qa-v2/mobile-${name}.png`)});
  }
 }
 if(width===1100)await page.screenshot({path:path.join(upper,'qa-v2/desktop-opening.png')});
 await page.close();
}
await browser.close();
await fs.writeFile(path.join(upper,'qa-v2/layout-check.json'),JSON.stringify(results,null,2)+'\n');
