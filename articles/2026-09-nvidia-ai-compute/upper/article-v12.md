...
企业要使用 AI，首先需要决定哪些工作自己做、哪些能力向服务商购买。以文字客服助手为例，企业可以购买现成的客服软件，配置业务资料与服务流程；也可以自行开发程序，通过 API，也就是程序调用接口，向模型服务发送客户问题、取得回复。后一种方式由服务商运行模型，企业无需为这部分推理自行购买 GPU、部署模型。[模型 API 服务示例](https://docs.modelstudio.console.alibabacloud.com/en/model-studio/what-is-model-studio)

如果企业选择自己部署模型，也有两条路径：购买设备，在自己的设备上运行；或者租用云端 GPU 服务器，远程安装软件和运行模型。租用算力省去了硬件采购，但在自行管理云服务器的模式下，模型部署和应用维护仍由团队负责。[云端 GPU 服务器示例](https://www.alibabacloud.com/help/en/egs/quick-reference)

**购买的是设备、计算资源，还是已经运行好的模型服务，决定了企业需要自己承担多少开发与维护工作。** 这些方式也可以组合：例如，先在自己的电脑上开发测试，再把模型部署到租用的云端服务器。具体服务形式与计费将在下篇展开。
