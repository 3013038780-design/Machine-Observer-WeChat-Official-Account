...
for(const width of [360,390,430,1100]){
 ...
 await page.goto(pathToFileURL(path.join(upper,'article-v8.html')).href);
 ...
 if(width===390){
  await page.screenshot({path:path.join(upper,'qa-v8/mobile-full.png'),fullPage:true});
  await page.screenshot({path:path.join(upper,'qa-v8/mobile-opening.png')});
  for(const [i,name] of [[0,'product-map'],[1,'architecture'],[2,'components'],[3,'launch-prices'],[4,'metrics'],[5,'workstations'],[6,'gpu-specs'],[7,'market-quotes']]){
   const el=page.locator('body>section>img').nth(i);const y=await el.evaluate(el=>el.getBoundingClientRect().top+scrollY);
   await page.evaluate(y=>scrollTo(0,Math.max(0,y-100)),y);
   await page.screenshot({path:path.join(upper,`qa-v8/mobile-${name}.png`)});
  }
 }
 if(width===1100)await page.screenshot({path:path.join(upper,'qa-v8/desktop-opening.png')});
 await page.close();
}
await browser.close();
await fs.writeFile(path.join(upper,'qa-v8/layout-check.json'),JSON.stringify(results,null,2)+'\n');
