...
for(const width of [360,390,430,1100]){
 ...
 if(width===390){
  ...
  for(const [i,name] of [[1,'gpu-specs'],[2,'launch-prices'],[3,'market-quotes']]){
   ...
   await page.screenshot({path:path.join(upper,`qa/mobile-${name}.png`)});
  }
 }
 if(width===1100)await page.screenshot({path:path.join(upper,'qa/desktop-opening.png')});
 await page.close();
}
await browser.close();
await fs.writeFile(path.join(upper,'qa/layout-check.json'),JSON.stringify(results,null,2)+'\n');
console.log(JSON.stringify(results.map(({paragraphs,...r})=>({...r,bodyFonts:[...new Set(paragraphs.map(p=>JSON.stringify(p)))]})),null,2));
if(results.some(r=>r.overflow.length||r.errors.length||r.badTags))process.exitCode=1;
