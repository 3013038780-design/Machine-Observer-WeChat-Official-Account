...
for(const width of [360,390,430,1100]){
 ...
 await page.goto(pathToFileURL(path.join(upper,'article-v3.html')).href);
 ...
 if(width===390){
  await page.screenshot({path:path.join(upper,'qa-v3/mobile-full.png'),fullPage:true});
  await page.screenshot({path:path.join(upper,'qa-v3/mobile-opening.png')});
  for(const [i,name] of [[0,'product-map'],[1,'architecture'],[2,'launch-prices'],[3,'workstations'],[4,'gpu-specs'],[5,'market-quotes']]){
   ...
   await page.screenshot({path:path.join(upper,`qa-v3/mobile-${name}.png`)});
  }
 }
 if(width===1100)await page.screenshot({path:path.join(upper,'qa-v3/desktop-opening.png')});
 await page.close();
}
await browser.close();
await fs.writeFile(path.join(upper,'qa-v3/layout-check.json'),JSON.stringify(results,null,2)+'\n');
