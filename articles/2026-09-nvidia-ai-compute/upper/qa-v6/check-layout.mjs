...
for(const width of [360,390,430,1100]){
 ...
 await page.goto(pathToFileURL(path.join(upper,'article-v6.html')).href);
 ...
 if(width===390){
  await page.screenshot({path:path.join(upper,'qa-v6/mobile-full.png'),fullPage:true});
  await page.screenshot({path:path.join(upper,'qa-v6/mobile-opening.png')});
  for(const [i,name] of [[0,'components'],[1,'metrics'],[2,'product-map'],[3,'architecture'],[4,'launch-prices'],[5,'workstations'],[6,'gpu-specs'],[7,'market-quotes']]){
   ...
   await page.screenshot({path:path.join(upper,`qa-v6/mobile-${name}.png`)});
  }
 }
 if(width===1100)await page.screenshot({path:path.join(upper,'qa-v6/desktop-opening.png')});
 await page.close();
}
await browser.close();
await fs.writeFile(path.join(upper,'qa-v6/layout-check.json'),JSON.stringify(results,null,2)+'\n');
